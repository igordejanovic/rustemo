...
extensions]] which are optional and build on top of a pure BNF. Rustemo is based
...
dealt with explicitly (see the [[#resolving-lr-conflicts][section on conflicts]]).
