# The Rustemo grammar language

```admonish warning
This document is ported from [the parglare
project](https://github.com/igordejanovic/parglare). Rustemo and this doc
are still work in progress and not all features of parglare are implemented yet.
```
...
sugar extensions](#syntactic-sugar-bnf-extensions) which are optional and builds
...
written declaratively. You don't have to think about the parsing process like in
e.g. [PEGs](https://en.wikipedia.org/wiki/Parsing_expression_grammar).
Ambiguities are dealt with explicitly (see the [section on
conflicts](./lr_parsing.md#resolving-conflicts)).

Each grammar file consists of three parts:
- zero or more imports of other grammar files.
...
your language -- tokens or lexemes (e.g. keywords, numbers). In Rustemo a
terminal is connected to the recognizer which is a type used to recognize token
of a particular kind in the input. Most of the time you will do parsing of
...
```admonish warning
TODO: Grammar comments are not implemented yet. For this to land a layout support
should be done.
```
