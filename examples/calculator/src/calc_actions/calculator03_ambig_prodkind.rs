...
use super::calculator03_ambig_prodkind_actions;
...
pub enum Terminal {
    ...
    LParen,
    RParen,
    Num(calculator03_ambig_prodkind_actions::Num),
}

#[derive(Debug)]
pub enum NonTerminal {
    E(calculator03_ambig_prodkind_actions::E),
}
...
pub struct Calculator03AmbigProdkindParserDefinition {
    actions: [[Action; TERMINAL_NO]; STATE_NO],
    gotos: [[Option<StateIndex>; NONTERMINAL_NO]; STATE_NO]
}

pub(in crate) static PARSER_DEFINITION: Calculator03AmbigProdkindParserDefinition = Calculator03AmbigProdkindParserDefinition {
    actions: [
    // State 0:AUG
...
impl ParserDefinition for Calculator03AmbigProdkindParserDefinition {
...
pub struct Calculator03AmbigProdkindParser(LRParser<Calculator03AmbigProdkindParserDefinition>);

impl<I, L, B> Parser<I, LRContext<I>, L, B> for Calculator03AmbigProdkindParser
where
    I: Debug,
    ...
    B: LRBuilder<I>,
{
    fn parse(&mut self, context: LRContext<I>, lexer: L, builder: B) -> Result<B::Output> {
        Calculator03AmbigProdkindParser::default().0.parse(context, lexer, builder)
    }
}

#[allow(dead_code)]
impl Calculator03AmbigProdkindParser
{
    pub fn parse_str<'i>(input: &'i str) -> Result<<Calculator03AmbigProdkindBuilder as Builder>::Output> {
        let context = LRContext::new("<str>".to_string(), input);
        let lexer = LRStringLexer::new(&LEXER_DEFINITION);
        let builder = Calculator03AmbigProdkindBuilder::new();
        Calculator03AmbigProdkindParser::default().0.parse(context, lexer, builder)
    }
}

impl Default for Calculator03AmbigProdkindParser {
    fn default() -> Self {
        Self(LRParser::new(&PARSER_DEFINITION))
    }
}

pub struct Calculator03AmbigProdkindLexerDefinition {
...
pub(in crate) static LEXER_DEFINITION: Calculator03AmbigProdkindLexerDefinition = Calculator03AmbigProdkindLexerDefinition {
    terminals: [
    TerminalInfo {
        id: TermIndex(0),
        name: "STOP",
...
impl LexerDefinition for Calculator03AmbigProdkindLexerDefinition {
...
pub struct Calculator03AmbigProdkindBuilder {
    res_stack: Vec<Symbol>,
}

impl Builder for Calculator03AmbigProdkindBuilder
{
    type Output = calculator03_ambig_prodkind_actions::E;

    fn new() -> Self {
        Calculator03AmbigProdkindBuilder {
            res_stack: vec![],
        }
    }
...
impl<'i> LRBuilder<&'i str> for Calculator03AmbigProdkindBuilder {
    ...
    fn shift_action(&mut self, term_idx: TermIndex, token: Token<&'i str>) {
        let termval = match TermKind::try_from(term_idx.0).unwrap() {
            ...
            TermKind::Num => Terminal::Num(calculator03_ambig_prodkind_actions::num(token)),
        };
        ...
        let prod = match ProdKind::try_from(prod_kind.0).unwrap() {
            ProdKind::EP0 => {
                ...
                match (i.next().unwrap(), i.next().unwrap(), i.next().unwrap()) {                
                    (Symbol::NonTerminal(NonTerminal::E(p0)), _, Symbol::NonTerminal(NonTerminal::E(p1))) => NonTerminal::E(calculator03_ambig_prodkind_actions::e_add(p0, p1)),
            ...
            ProdKind::EP1 => {
                let mut i = self.res_stack.split_off(self.res_stack.len()-3).into_iter();
                match (i.next().unwrap(), i.next().unwrap(), i.next().unwrap()) {                
                    (Symbol::NonTerminal(NonTerminal::E(p0)), _, Symbol::NonTerminal(NonTerminal::E(p1))) => NonTerminal::E(calculator03_ambig_prodkind_actions::e_sub(p0, p1)),
                    _ => panic!("Invalid symbol parse stack data.")
                }
            },
            ProdKind::EP2 => {
                let mut i = self.res_stack.split_off(self.res_stack.len()-3).into_iter();
                match (i.next().unwrap(), i.next().unwrap(), i.next().unwrap()) {                
                    (Symbol::NonTerminal(NonTerminal::E(p0)), _, Symbol::NonTerminal(NonTerminal::E(p1))) => NonTerminal::E(calculator03_ambig_prodkind_actions::e_mul(p0, p1)),
                    _ => panic!("Invalid symbol parse stack data.")
                }
            },
            ProdKind::EP3 => {
                let mut i = self.res_stack.split_off(self.res_stack.len()-3).into_iter();
                match (i.next().unwrap(), i.next().unwrap(), i.next().unwrap()) {                
                    (Symbol::NonTerminal(NonTerminal::E(p0)), _, Symbol::NonTerminal(NonTerminal::E(p1))) => NonTerminal::E(calculator03_ambig_prodkind_actions::e_div(p0, p1)),
                    _ => panic!("Invalid symbol parse stack data.")
                }
            },
            ProdKind::EP4 => {
                let mut i = self.res_stack.split_off(self.res_stack.len()-3).into_iter();
                match (i.next().unwrap(), i.next().unwrap(), i.next().unwrap()) {                
                    (Symbol::NonTerminal(NonTerminal::E(p0)), _, Symbol::NonTerminal(NonTerminal::E(p1))) => NonTerminal::E(calculator03_ambig_prodkind_actions::e_pow(p0, p1)),
                    _ => panic!("Invalid symbol parse stack data.")
                }
            },
            ProdKind::EP5 => {
                let mut i = self.res_stack.split_off(self.res_stack.len()-3).into_iter();
                match (i.next().unwrap(), i.next().unwrap(), i.next().unwrap()) {                
                    (_, Symbol::NonTerminal(NonTerminal::E(p0)), _) => NonTerminal::E(calculator03_ambig_prodkind_actions::e_paren(p0)),
                    _ => panic!("Invalid symbol parse stack data.")
                }
            },
            ProdKind::EP6 => {
                ...
                match i.next().unwrap() {                
                    Symbol::Terminal(Terminal::Num(p0)) => NonTerminal::E(calculator03_ambig_prodkind_actions::e_num(p0)),
                    _ => panic!("Invalid symbol parse stack data.")
                }
            },
        };
