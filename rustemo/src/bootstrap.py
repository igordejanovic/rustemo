...
if __name__ == '__main__':
    ...
    with open('rustemo.rs', 'w') as f:
        ...
        f.write('use rustemo_rt::lexer::{Lexer, Token};\n')
        f.write('use rustemo_rt::parser::Parser;\n')
        f.write('use rustemo_rt::error::RustemoResult;\n')
        f.write('use rustemo_rt::lr::lexer::{LRStringLexer, LRContext, LexerDefinition, RecognizerIterator};\n')
        f.write('use rustemo_rt::lr::builder::LRBuilder;\n')
        f.write('use rustemo_rt::lr::parser::{LRParser, ParserDefinition};\n')
        f.write('use rustemo_rt::lr::parser::Action::{self, Shift, Reduce, Accept, Error};\n')
        f.write('use rustemo_rt::index::{StateIndex, TermIndex, NonTermIndex, ProdIndex};\n')
        f.write('use rustemo_rt::builder::Builder;\n')
        f.write('use rustemo_rt::grammar::{TerminalInfo, TerminalInfos, TerminalsState};\n')
        f.write('use rustemo_rt::debug::{log, logn};\n')
        ...
        f.write('pub struct RustemoParser(LRParser<RustemoParserDefinition>);\n\n');

        f.write('impl Default for RustemoParser {\n')
        f.write('    fn default() -> Self {\n')
        f.write('        Self(LRParser::new(&PARSER_DEFINITION))\n')
        f.write('    }\n')
        f.write('}\n\n')

        f.write('impl RustemoParser {\n')
        f.write('    pub fn parse<I, L, B>(mut context: LRContext<I>, lexer: L, mut builder: B) -> RustemoResult<B::Output>\n')
        f.write('    where\n')
        f.write('        I: Debug,\n')
        f.write('        L: Lexer<I, LRContext<I>>,\n')
        f.write('        B: LRBuilder<I>,\n')
        f.write('    {\n')
        f.write('        Self::default().0.parse(context, lexer, builder)\n')
        f.write('    }\n\n')
        f.write('    pub fn parse_str<\'i>(input: &\'i str) -> RustemoResult<<RustemoBuilder as Builder>::Output> {\n')
        f.write('        let context = LRContext::new("<str>".to_string(), input);\n')
        f.write('        let lexer = LRStringLexer::new(&LEXER_DEFINITION);\n')
        f.write('        let builder = RustemoBuilder::new();\n')
        f.write('        RustemoParser::default().0.parse(context, lexer, builder) \n')
        f.write('    }\n\n')
        f.write('    // FIXME: Return/move owned input string with the result.\n')
        f.write('    // pub fn parse_file<F: AsRef<Path>>(file: F) -> RustemoResult<<RustemoBuilder as Builder>::Output> {\n')
        f.write('    //    let input = std::fs::read_to_string(file.as_ref())?;\n')
        f.write('    //    let context = LRContext::new(file.as_ref().to_str().unwrap().to_string(), input);\n')
        f.write('    //    let lexer = LRStringLexer::new(&LEXER_DEFINITION);\n')
        f.write('    //    let builder = RustemoBuilder::new();\n')
        f.write('    //    RustemoParser::default().0.parse(context, lexer, builder) \n')
        f.write('    //}\n')
        ...
        f.write('pub struct RustemoBuilder {\n')
        f.write('    res_stack: Vec<Symbol>,\n')
        f.write('}\n')
        f.write('\n')
        f.write('impl Builder for RustemoBuilder\n')
        f.write('{\n')
        ...
        f.write('        }\n')
        f.write('    }\n\n')
        f.write('    fn get_result(&mut self) -> RustemoResult<Self::Output> {\n')
        f.write('       Ok(self.res_stack.pop().unwrap())\n')
        f.write('    }\n')
        f.write('}\n\n')
        f.write('impl<\'i> LRBuilder<&\'i str> for RustemoBuilder\n')
        f.write('{\n')
        f.write('    fn shift_action(&mut self, term_idx: TermIndex, token: Token<&\'i str>) {\n')
