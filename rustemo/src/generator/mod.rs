    ...
    F: AsRef<Path> + Debug,
{
    ...
    file_name = String::from(
        file_name
            .strip_suffix(".rustemo")
            .expect("Grammar filename must use .rustemo extension."),
    );
    ...
    let mut out_file = PathBuf::from(grammar_path.as_ref());
    out_file.pop();
    out_file.push(file_name + "_types.rs");
    let out_file = File::create(out_file).unwrap();
    generate_parser_types(&grammar, out_file)?;
...
fn generate_parser_definition<W: Write>(
    ...
    geni!(
        ...
        impl<'i> Parser<L, B> for {parser_name}Parser<'i>
        where
            L: Lexer<Input = &'i str>,
            B: Builder<Lexer = L>
        {{
...
fn generate_parser_types<W: Write>(
    ...
    for production in &grammar.productions()[1..] {
        ...
        geni!(out, "{}P{} = {},\n", nt.name, production.ntidx, production.idx);
    }
    out.dec_indent();
    geni!(out, "}}\n\n");
