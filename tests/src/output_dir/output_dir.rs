...
use rustemo::index::{NonTermIndex, ProdIndex, StateIndex, TermIndex};
use rustemo::lexer::{self, AsStr, Token};
...
pub(crate) static PARSER_DEFINITION: OutputDirParserDefinition =
    OutputDirParserDefinition {
        actions: [
            ...
            [
                Reduce(ProdIndex(1usize), 2usize, NonTermIndex(2usize)),
                Error,
                Error,
            ],
            [
...
impl OutputDirParser {
    ...
    pub fn parse_file<'i, P: AsRef<std::path::Path>>(
        ...
        self.content =
            Some(<Input as rustemo::lexer::Input>::read_file(&file)?);
...
pub(crate) static LEXER_DEFINITION: OutputDirLexerDefinition =
    OutputDirLexerDefinition {
        terminals_for_state: [
            [Some(1usize), None],
...
impl LexerDefinition for OutputDirLexerDefinition {
    ...
    fn recognizers(
        ...
        RecognizerIterator {
            terminals_for_state: &LEXER_DEFINITION.terminals_for_state
                [state_index.0][..],
...
impl<'i> LRBuilder<'i, Input, TokenKind> for OutputDirBuilder {
    ...
    fn shift_action(
        ...
        let val = match kind {
            ...
            TokenKind::Num => {
                Terminal::Num(output_dir_actions::num(context, token))
            }
        };
        ...
        let prod = match ProdKind::from(prod_idx) {
            ProdKind::AP1 => {
                ...
                match (i.next().unwrap(), i.next().unwrap()) {
                    ...
                    ) => NonTerminal::A(output_dir_actions::a_c1(
                        context, p0, p1,
                    )),
            ...
            ProdKind::B1P1 => {
                ...
                match (i.next().unwrap(), i.next().unwrap()) {
                    ...
                    ) => NonTerminal::B1(output_dir_actions::b1_c1(
                        context, p0, p1,
                    )),
                    _ => panic!("Invalid symbol parse stack data."),
                }
